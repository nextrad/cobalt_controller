...
N_waveforms=2*length(T_paramVec);
B1_NLFM_param= 50e6;
B2_NLFM_param= 150e6;

for ind_wav=1:N_waveforms;
    
    if ind_wav <= length(T_paramVec);
...
N = length(t)+15;  % 15 is a magical number. Come up with something better if you like.
N = N-mod(N,8)-1   % Ensures that N is a multiple of 8 minus 1
...
%v_txa = [  zeros(1,1+N-length(v_txa)) v_txa            zeros(1,8)     ]; % pad v_txa with a few zeros
%v_txa = [  zeros(1,N-length(v_txa)) v_txa              zeros(1,9)     ]; % pad v_txa with a few zeros
...
%generate NLFM chirp attached 
    
i = ind_wav-length(T_paramVec);
T_param = T_paramVec(i);
...
K_NLFM = [B1_NLFM_param/T B2_NLFM_param/T^3];    % Chirp rates
td = T/2;   % shifting the pulse by T/2
rect =@(t) abs(t) <= 0.5;
phase_NLFM = 1i*2*pi.*f0*t+1i*pi.*K_NLFM(1)*t.^2+1i*pi.*K_NLFM(2)*t.^4;

v_tx = real(exp(phase_NLFM)).*rect((t)/T); % Skolnik eq 8.1
v_txa = exp(phase_NLFM).*rect((t)/T); % Skolnik eq 8.1 %v_txa= hilbert(real(v_tx));
...
T_paramVec = [T_paramVec T_paramVec];
...
V_tx = fftshift(fft(v_tx_lfm,NFFT)/L);
%V_txa = fftshift(fft(v_txa,NFFT)/L);
V_txa = fftshift(fft((v_txa_lfm),NFFT)/L);
